...
/**
 * @file        fglconfig.h
 * @brief       This is the rasterizer-setting file.
 * @author      Inho Lee
 * @version     1.5
 */
...
/***************************************************************************** 
 * FUNCTIONS: fglSetDepthOffsetParamf
 *
 *                      refer to
 ...
 *             [IN] float value: specified parameter value
 * RETURNS: FGL_ERR_NO_ERROR - if successful.
 *          FGL_ERR_INVALID_VALUE - if an invalid parameter is specified.
 * ERRNO: FGL_ERR_INVALID_VALUE = 7
 *****************************************************************************/
FGL_Error
fglSetDepthOffsetParamf(FGL_DepthOffsetParam param, float value)
{
        switch (param)
        {
        case (FGL_DEPTH_OFFSET_FACTOR) : 
                WRITEREGF(FGRA_DEPTH_OFFSET_FACTOR, value);
                return FGL_ERR_NO_ERROR;
        case (FGL_DEPTH_OFFSET_UNITS) :
                WRITEREGF(FGRA_DEPTH_OFFSET_UNIT, value);
                return FGL_ERR_NO_ERROR;
        case (FGL_DEPTH_OFFSET_R) :
                WRITEREGF(FGRA_DEPTH_OFFSET_RVAL, value);
                return FGL_ERR_NO_ERROR;
        default :
                return FGL_ERR_INVALID_VALUE;
        }
