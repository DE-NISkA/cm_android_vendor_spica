...
char *lib_version = "1.3 (lib version 13.02.22)";
...
void copyBuffer( void *dstBuffer, void *srcBuffer, EGLint dstWidth, EGLint dstHeight, EGLint srcWidth, EGLint srcHeight, EGLint bufferBytes )
{
	...
	srcBuffer16+= srcWidth*(srcHeight-1);
        ...
        {
            PLATFORM_MEMCPY( dstBuffer16, srcBuffer16, 2 * minWidth );
    ...
    else if( bufferBytes == 4 )
    {
        for( i = 0; i < minHeight; i++ )
        {
            PLATFORM_MEMCPY( dstBuffer32, srcBuffer32, 4 * minWidth );
...
void resizeWindowSurface(EGLSurface surface, EGLint width, EGLint height)
{
...
void sortingConfigs( EGLConfig** configs, EGLint num_config )
{
    EGLint i, j;
    EGLint tmpPriority;
    ...
    configAtt = (EGLConfigAtt **)PLATFORM_MALLOC( sizeof(EGLConfigAtt *) * num_config );
    priority  = (EGLint*)PLATFORM_MALLOC( sizeof(EGLint) * num_config );

    PLATFORM_MEMSET( configAtt, 0x00, sizeof(EGLConfigAtt *) * num_config );
    PLATFORM_MEMSET( priority,  0x00, sizeof(EGLint) * num_config );

    for( i = 0; i < num_config; i++ )
        configAtt[i] = (*configs)[i];
...
EGLDisplay eglGetDisplay(EGLNativeDisplayType displayID)
{
     ...
     if (!_egl.init)
    {
        ...
        _egl.configs = (EGLConfig *)PLATFORM_MALLOC( sizeof(EGLConfig) * _egl.num_config );

        _egl.defaultConfig = (EGLConfigAtt *)PLATFORM_MALLOC( sizeof(EGLConfigAtt) * _egl.num_config );
        PLATFORM_MEMSET( _egl.defaultConfig, 0x00, sizeof(EGLConfigAtt) * _egl.num_config );
    ...
    if (displayID == EGL_DEFAULT_DISPLAY)
        EGL_ERROR( EGL_SUCCESS, _egl.displays[0] );
...
const char* eglQueryString(EGLDisplay dpy, EGLint name)
{
    ...
    switch ( name )
    {
        case EGL_CLIENT_APIS:
			LOGI("EGL_CLIENT_APIS: OpenVG OpenGL_ES");
            EGL_ERROR( EGL_SUCCESS, "OpenVG OpenGL_ES" );

        case EGL_VENDOR:
			LOGI("EGL_VENDOR     : HUONE");
            EGL_ERROR( EGL_SUCCESS, "HUONE" );

        case EGL_VERSION:
			LOGI("EGL_VERSION    : 1.3");
            EGL_ERROR( EGL_SUCCESS, "1.3" );

        case EGL_EXTENSIONS:
			LOGI("EGL_EXTENSIONS :no extension");
            EGL_ERROR( EGL_SUCCESS, "" );

        default:
			LOGE("query string error");
    ...
    *num_config = _egl.num_config;
    ...
    if ( !attrib_list )
    {
        ...
        *num_config = EGL_MIN( config_size, j );
        ...
        if( *num_config >= 2 )
            sortingConfigs( &configs, *num_config );
            ...
            for ( k = 0; attrib_list[k] != EGL_NONE; k += 2 )
            {
                ...
                switch( attrib_list[k] )
                {
                    ...
                    case EGL_BIND_TO_TEXTURE_RGB:
						if ( configAtt->eglBindToTextureRGB != attrib_list[k + 1] && boundAPI == EGL_OPENGL_ES_API )
                            flag = EGL_FALSE;
                        break;
                    case EGL_BIND_TO_TEXTURE_RGBA:
						if ( configAtt->eglBindToTextureRGBA != attrib_list[k + 1] && boundAPI == EGL_OPENGL_ES_API )
                            flag = EGL_FALSE;
                        break;
                    case EGL_COLOR_BUFFER_TYPE:
                        if ( configAtt->eglColorBufferType != attrib_list[k + 1] )
                            flag = EGL_FALSE;
                        break;
                    case EGL_CONFIG_CAVEAT:
                        if ( configAtt->eglConfigCaveat != attrib_list[k + 1] )
                            flag = EGL_FALSE;
                        break;
                    case EGL_CONFIG_ID:
                    ...
                    case EGL_SURFACE_TYPE:
                        ...
                        break;
                    case EGL_TRANSPARENT_TYPE:
                        if ( configAtt->eglTransparentType != attrib_list[k + 1] )
                            flag = EGL_FALSE;
                        break;
...
//    EGLint        width = 0, height = 0;
    ...
    //if( !win )
    if( !PLATFORM_IS_WINDOW_VALID( win ) )
    ...
    if ( !(surface = (iEGLSurface *)PLATFORM_MALLOC( sizeof(iEGLSurface) )) )
        EGL_ERROR( EGL_BAD_ALLOC, EGL_NO_SURFACE );


	PLATFORM_MEMSET( surface, 0x00, sizeof(iEGLSurface) );

    if( !(surface->backBuffer1 = (AddressBase *)PLATFORM_MALLOC( sizeof(AddressBase) )) )
    {
    ...
    if( !(surface->depthBuffer = (AddressBase *)PLATFORM_MALLOC( sizeof(AddressBase) )) )
    {
        PLATFORM_FREE( surface->backBuffer1 );
#if defined(DOUBLE_BUFFER)
        PLATFORM_FREE( surface->backBuffer2 );
    ...
    PLATFORM_MEMSET( surface->backBuffer1, 0x00, sizeof(AddressBase) );
#if defined(DOUBLE_BUFFER)
...
#endif
    PLATFORM_MEMSET( surface->depthBuffer, 0x00, sizeof(AddressBase) );

	surface->width = win->width;
	surface->height = win->height;
	...
	{
		AddressBase Surface_addr;
		get_addressbase(&Surface_addr);

		if(!Surface_addr.paddr)
		{
#if !defined(DIRECT_WINDOW_DRAWING)
#if defined(DOUBLE_BUFFER)
		...
		{
#if defined(DOUBLE_BUFFER)
			surface->backBuffer1->paddr = Surface_addr.paddr;
			surface->backBuffer1->vaddr = Surface_addr.vaddr;
//			LOGE("####################backbuffer2 vaddr = 0x%x, paddr = 0x%x ###########", surface->backBuffer2->vaddr, surface->backBuffer2->paddr);
//			win->nextBuffer(win);
			surface->backBuffer2->paddr = Surface_addr.paddr;
			surface->backBuffer2->vaddr = Surface_addr.vaddr;
//			LOGE("####################backbuffer1 vaddr = 0x%x, paddr = 0x%x ###########", surface->backBuffer1->vaddr, surface->backBuffer1->paddr);
    ...
    PLATFORM_WINDOW_SURFACE_INITIALIZE( dpy, win );
...
EGLSurface eglCreatePixmapSurface(EGLDisplay dpy, EGLConfig config, NativePixmapType pixmap, const EGLint* attrib_list)
{
    ...
    //if ( !pixmap )
    if( !PLATFORM_IS_PIXMAP_VALID( pixmap ) )
    ...
    PLATFORM_GET_PIXMAP_COLORBUFFER( pixmap, &surface->backBuffer1->vaddr, &surface->backBuffer1->paddr );
...
EGLBoolean eglDestroySurface(EGLDisplay dpy, EGLSurface surface)
{
    ...
    switch( iSurface->surfaceType )
    {
    case EGL_WINDOW_BIT :

	if(!iSurface->direct_fb){
#if !defined(DIRECT_WINDOW_DRAWING)
#if defined(DOUBLE_BUFFER)
...
//		release_gpuaddress();
        ...
        PLATFORM_WINDOW_SURFACE_FINALIZE( dpy, iSurface->window );
    ...
    case EGL_PIXMAP_BIT :
        {
            ...
            for( i = 0; configAtt->associatedPixmap[i]; i++ )
            {
                if( configAtt->associatedPixmap[i] == iSurface->window )
                {
...
EGLBoolean eglQuerySurface(EGLDisplay dpy, EGLSurface surface, EGLint attribute, EGLint* value)
{
    ...
    switch( attribute )
    {
        ...
        case EGL_BITMAP_POINTER_KHR:
            value = (EGLint *)iSurface->bitmapPointer;
            iSurface->bitmapPointer = iSurface->backBuffer1;
            break;
...
EGLContext eglCreateContext( EGLDisplay dpy, EGLConfig config, EGLContext share_list, const EGLint* attrib_list )
{
    ...
    EGLThreadState *thread;
    iEGLContext *sharedContext = share_list;
    ...
    if ( !(context = (iEGLContext *)PLATFORM_MALLOC(sizeof(iEGLContext))) )
        EGL_ERROR( EGL_BAD_ALLOC, EGL_NO_CONTEXT );

	PLATFORM_MEMSET( context, 0x00, sizeof(iEGLContext) );
...
EGLBoolean eglMakeCurrent(EGLDisplay dpy, EGLSurface draw, EGLSurface read, EGLContext ctx)
{
	...
	if( (draw && (((iEGLSurface*)draw)->surfaceType == EGL_WINDOW_BIT) && !PLATFORM_IS_WINDOW_VALID(((iEGLSurface*)draw)->window)) ||
		(read && (((iEGLSurface*)read)->surfaceType == EGL_WINDOW_BIT) && !PLATFORM_IS_WINDOW_VALID(((iEGLSurface*)read)->window)) )
    ...
    if( (drawSurface->surfaceType == EGL_WINDOW_BIT && !PLATFORM_IS_WINDOW_VALID(drawSurface->window)) )
        EGL_ERROR( EGL_BAD_NATIVE_WINDOW, EGL_FALSE );
...
EGLBoolean eglSwapBuffers(EGLDisplay dpy, EGLSurface surface)
{
    ...
    if( iSurface->surfaceType == EGL_WINDOW_BIT && window->memory_type!=2)
    {
        switch( boundAPI )
        {
        case EGL_OPENVG_API:
    ...
    {
		LOGE("need to resizeWindowSurface ");
	// 	resizeWindowSurface( (EGLSurface)iSurface, width, height );
		...
		case EGL_OPENVG_API:
            ...
            case EGL_OPENGL_ES_API:
                //eglGLESSetColorBuffer( iSurface );
    ...
    if( iSurface->surfaceType == EGL_WINDOW_BIT )
    {
        ...
        {
            iSurface->frontBuffer = iSurface->backBuffer2;
            iSurface->curDrawingBuffer = 3;
        }
        else
        {
            iSurface->frontBuffer = iSurface->backBuffer3;
            iSurface->curDrawingBuffer = 1;
        }
    }
    else
...
#if !defined(DIRECT_WINDOW_DRAWING)
	if(iSurface->direct_fb == 0)
	PLATFORM_DISPLAY_BUFFER( dpy, window->base+window->offset, iSurface->frontBuffer->vaddr, iSurface->width, iSurface->height, iSurface->bufferBytes );
#if 0
	else{
#ifdef CACHE_MEM
		if(window->memory_type == 1)
		PLATFORM_CACHE_INVALID_RANGE(window->base+window->offset,width*height*2);
#endif
	}
#endif

#else
	PLATFORM_DISPLAY_BUFFER( dpy, iSurface->window, 0, iSurface->width, iSurface->height, iSurface->bufferBytes );
#endif

//LOGE("### type=%d base=%x offset=%x w=%d h=%d",window->memory_type,window->base,window->offset,iSurface->width,iSurface->height);
	releaseMutex();
	window->swapBuffers(window);
	acquireMutex();

	if(window->offset && iSurface->direct_fb==1 && window->memory_type == 1){
		iSurface->backBuffer2->paddr = iSurface->backBuffer1->paddr+window->offset;
		iSurface->backBuffer2->vaddr = iSurface->backBuffer1->vaddr+window->offset;
	}
...
EGLBoolean eglCopyBuffers(EGLDisplay dpy, EGLSurface surface, NativePixmapType target)
{
    ...
    PLATFORM_COPY_BUFFER( dpy, iSurface->frontBuffer->vaddr, target, iSurface->width, iSurface->height );
...
void (*eglGetProcAddress(const char *procname)) (void)
{
	...
	if(!PLATFORM_STRCMP( procname, "eglLockSurfaceKHR", 17))
		EGL_ERROR( EGL_SUCCESS, (void *)eglLockSurfaceKHR );

	if(!PLATFORM_STRCMP( procname, "eglUnlockSurfaceKHR", 19))
		EGL_ERROR( EGL_SUCCESS, (void *)eglUnlockSurfaceKHR );

	if(!PLATFORM_STRCMP( procname, "eglCreateImageKHR", 17))
		EGL_ERROR( EGL_SUCCESS, (void *)eglCreateImageKHR );

	if(!PLATFORM_STRCMP( procname, "eglDestroyImageKHR", 18))
		EGL_ERROR( EGL_SUCCESS, (void *)eglDestroyImageKHR );

    EGL_ERROR( EGL_SUCCESS, (void *)EGL_FALSE );
}
...
EGLImageKHR eglCreateImageKHR( EGLDisplay dpy, EGLContext ctx, EGLenum target, EGLClientBuffer buffer, EGLint* attr_list)
{
	...
	image = (iEGLImage *)PLATFORM_MALLOC( sizeof(iEGLImage) );
    PLATFORM_MEMSET( image, 0xFF, sizeof(iEGLImage) );
