...
inline uint16_t bswap_16 (uint16_t v)   { asm ("rorw $8, %w0" : "=r"(v) : ""(v) : "cc"); return (v); }
inline uint32_t bswap_32 (uint32_t v)   { asm ("bswap %0" : "=r"(v) : ""(v)); return (v); }
#else
...
inline bool TestAndSet (int* pm)
{
    ...
    int oldVal (1);
    asm volatile ("xchgl %0, %1" : "=r"(oldVal), "=m"(*pm) : ""(oldVal), "m"(*pm) : "memory");
    return (!oldVal);
